...
function MiniChart({ story, timeRange }: MiniChartProps) {
...
export default function CompareIndicatorsPage() {
    const [selectedIndicators, setSelectedIndicators] = useState<string[]>([
        'fear-greed', 'funding-rate', 'long-short', 'liquidation'
    ]);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {selectedStories.map(story => (
                        <Link key={story.id} href={`/indicators/${story.slug}`}>
                            <MiniChart story={story} timeRange={timeRange} />
                        </Link>
                    ))}
                </div>
