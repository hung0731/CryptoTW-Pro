...
export default function ArticlesAdminPage() {
    ...
    const [formData, setFormData] = useState<Partial<Article>>({
        title: '',
        body: '',
        type: 'alpha', // Default to alpha
        access_level: 'pro', // Default to pro for articles
        is_published: true
    })
    ...
    const fetchArticles = async () => {
        ...
        try {
            ...
            if (data.content) {
                // Filter Alpha & Weekly
                setArticles(data.content.filter((a: Article) => a.type === 'alpha' || a.type === 'weekly'))
            }
        } catch (e) {
            console.error(e)
        } finally {
    ...
    return (
        <div className="p-6 md:p-8 space-y-8 w-full">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-white">文章管理 (Articles)</h1>
                    <p className="text-neutral-400 mt-2">管理 Pro 觀點與週報內容。</p>
                </div>
