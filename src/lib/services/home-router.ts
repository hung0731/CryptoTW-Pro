...
export class HomeRouterService {
    static async getRouterData(forceRefresh = false): Promise<HomeRouterData> {
        return withCache(
            CACHE_KEYS.HOME_ROUTER_DATA,
            CACHE_TTL.FAST, // 5 mins
            async () => {
                // Parallel fetch
                ...
                let marketContext: MarketContext | null = await getCache(CACHE_KEYS.MARKET_CONTEXT)
                let aiDecision: AIDecision | null = await getCache(CACHE_KEYS.MARKET_DECISION)
                ...
                if (!aiDecision) {
                    ...
                    decisionPromise = generateAIDecision({
                        ...
                        if (res) await setCache(CACHE_KEYS.MARKET_DECISION, res, 900) // 15 mins cache
                        return res
                    })
                }
