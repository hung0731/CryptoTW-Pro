...
【輸出要求】
1. **自然語言分析**：不要用條列式或「劇本A/B」的僵硬格式。
2. **連貫敘事**：像一位資深分析師在對話，直接說明市場預期，並自然地延伸出「若數據高於預期...」或「若低於預期...」的推演。
3. **風格**：專業、流暢、科技感、帶有警示性。
4. **推薦關注**：針對接下來的事件，推薦 2 個日曆頁面。
...
【輸出格式】JSON
{
    "summary": "請將「預期鋪墊」與「情境推演」融合成一段流暢的自然語言 (約 100-120 字)。\n範例：「本週非農數據將是關鍵分水嶺，市場目前預期就業放緩。若數據意外強勁，可能引發對於降息延後的恐慌，導致比特幣回測支撐；反之，若數據疲軟，則有望點燃多頭情緒，挑戰前高壓力區。」",
    "recommended_readings": [
