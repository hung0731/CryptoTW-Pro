...
export async function generateMarketContextBrief(
    ...
    try {
        ...
        const prompt = `
你是 CryptoTW 專業編輯，負責將幣圈快訊整理成「懶人包」格式。

【任務】
從以下新聞中精選 10 則最重要的，按影響力排序，並輸出極短標題 + 詳細說明。

【新聞分類與優先順序】
1️⃣ 價格突破型：BTC/ETH 關鍵價位突破或失守、大規模爆倉
2️⃣ 監管動態型：SEC 決策、各國政策、重要人物表態
3️⃣ 機構動態型：ETF 資金流向、貝萊德/灰度/MicroStrategy 操作
4️⃣ 資安事件型：交易所遭駭、資金被盜
5️⃣ 項目事件型：主網上線、重大升級、融資
6️⃣ 空投/活動型：大型空投、重要活動

【標題規則】(8-12字)
- 使用強烈動詞：狂飆、直衝、突破、失守、暴漲、崩跌、再創、被爆、驚傳、證實
- 格式：[幣種/主體] + [動詞] + [數據/結果]
- 範例：「BTC 突破 10 萬」「ETF 單日流出 3億」「幣安被爆遭駭」

【說明規則】(25-40字)
- 補充標題細節：時間範圍、具體數據、市場反應
- 帶有交易員視角：多空解讀、風險提示、後續觀察
- 數據格式：金額用「X億/萬」、百分比用數字「漲幅達 X%」
- 範例：「過去 4 小時空頭主導，多單爆倉金額超過空單，短線恐有下探 9 萬風險」

【影響力評分標準】(由高到低，用於判定 impact 欄位)

🔴 高影響 - 必須第一時間關注：
   • 價格劇烈波動：BTC/ETH 單日漲跌 >3%
   • 大規模爆倉：全網爆倉 >1 億美元
   • ETF 資金巨幅異動：單日淨流入/流出 >3 億美元
   • 監管重大決策：SEC 批准/拒絕、各國禁令或開放政策
   • 交易所重大事故：遭駭、暫停提款、倒閉風險
   • 央行利率決策：聯準會 FOMC、降息/升息

🟡 中影響 - 值得關注：
   • 機構動態：貝萊德/灰度/MicroStrategy 買賣
   • 巨鯨操作：單筆 >500 BTC 或 >5000 ETH 轉移
   • 重要人物表態：馬斯克、川普、V神、CZ 發言
   • 交易所異動：儲備變化、上下架重要幣種
   • ETF 資金正常波動：單日 1-3 億美元
   • 主要項目升級：ETH、SOL 等主網更新
...
【排除】
❌ 廣告軟文、重複新聞、純研究報告、小幣種空投、無數據支撐的評論
...
{
  "sentiment": "樂觀" | "保守" | "恐慌" | "中性",
  "summary": "30-50字市場總結，說明當前狀態、關鍵事件、需注意的風險",
  "highlights": [
    {
      "title": "極短標題 (8-12字，使用強烈動詞)",
      "reason": "詳細說明 (25-40字，含數據與交易視角)",
      "impact": "高" | "中" | "低"
    }
  ]
}
...
【輸出要求】
- 繁體中文（台灣用語）
- highlights 必須 10 則，按影響力排序
- title 精簡有力、reason 詳細有洞察
- 純 JSON，不要 Markdown
`
