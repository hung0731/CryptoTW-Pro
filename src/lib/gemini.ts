...
export async function generateMarketContextBrief(
    ...
    try {
        ...
        const prompt = `
你是一個幣圈快訊篩選 AI。你的任務：從 Coinglass API 的快訊中，找出過去 24 小時「對市場影響最大」的新聞，並依嚴重程度排序成懶人包。

【任務】
1. 從以下新聞中篩選出「對幣價影響最大」的 10 則
2. 按「市場影響嚴重程度」由高到低排序
3. 輸出一句話懶人包總結

【排序規則】(由重到輕)
🔴 1. 價格劇烈波動：大幅爆倉、ETF 資金異動、巨鯨操作
🟠 2. 監管政策：SEC 動態、各國法規變化
🟡 3. 機構動態：貝萊德、灰度、MicroStrategy 等
🟢 4. 項目事件：主網上線、空投、駭客攻擊、融資
🔵 5. 名人動態：馬斯克、川普、V神發言
⚪ 6. 市場熱點：Meme 幣、鏈上數據異動

【排除】
❌ 廣告、軟文、技術更新細節、小額交易、重複內容

【輸入新聞 (Coinglass API)】
${JSON.stringify(newsItems.slice(0, 50).map(n => ({
            title: n.newsflash_title || n.title,
            content: (n.newsflash_content || n.content || '').slice(0, 150)
        })), null, 2)}

【輸出格式】(Strict JSON)
{
  "sentiment": "樂觀" | "保守" | "恐慌" | "中性",
  "summary": "20-30字，一句話市場懶人包 (例：ETF 資金持續流入，短線偏多但需注意爆倉風險)",
  "highlights": [
    {
      "title": "精簡標題 (15字內)",
      "reason": "對市場的影響 (10字內)",
...
輸出 JSON，不要 Markdown。highlights 必須有 10 則，按影響程度排序。
`
